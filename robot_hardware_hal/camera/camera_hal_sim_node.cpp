...
class CameraHalSimNode : public rclcpp::Node
{
public:
    CameraHalSimNode() : Node("camera_hal_sim_node")
    {
        sim_image_topic_ = this->declare_parameter<std::string>("sim_image_topic", "/image_raw");
        sim_camera_info_topic_ = this->declare_parameter<std::string>("sim_camera_info_topic", "/camera_info");
        ...
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            sim_image_topic_, qos,
            std::bind(&CameraHalSimNode::onImage, this, std::placeholders::_1));
        
        camera_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
            sim_camera_info_topic_, qos,
            std::bind(&CameraHalSimNode::onCameraInfo, this, std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(), "camera_hal_sim started");
        RCLCPP_INFO(this->get_logger(), "in: image=%s, info=%s",
            sim_image_topic_.c_str(), sim_camera_info_topic_.c_str());
    ...
    std::string sim_camera_info_topic_;
...
int main(int argc, char ** argv)
{
    ...
    rclcpp::spin(std::make_shared<CameraHalSimNode>());
    rclcpp::shutdown();
    return 0;
}